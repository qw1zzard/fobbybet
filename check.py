...
async def check(user_id):
    ...
    next_event = requests.get(url).json()['MRData']
    next_event = next_event['RaceTable']['Races'][0]['date']

    query = 'SELECT driver_id, bet_value FROM bets ' \
            'WHERE event_date = :next_event AND user_id = :user_id;'
    next_info = await database.fetch_all(query=query,
    ...
    last_event = requests.get(url).json()['MRData']
    last_event = last_event['RaceTable']['Races'][0]['date']

    query = 'SELECT driver_id, bet_value, payout_flag FROM bets ' \
            'WHERE event_date = :last_event AND user_id = :user_id;'
    last_info = await database.fetch_all(query=query,
                                         values={'last_event': last_event,
                                                 'user_id': user_id})
    ...
    if last_info:
        ...
        winner_id = requests.get(url).json()['MRData']['RaceTable']['Races'][0]
        winner_id = winner_id['Results'][0]['Driver']['permanentNumber']

        if last_driver_id == winner_id and payout_flag == 'True':
            query = 'SELECT bet_value, driver_id FROM bets ' \
                    'WHERE event_date = :last_event AND user_id != :user_id;'
            last_info = await database.fetch_all(query=query, values={
                'last_event': last_event, 'user_id': user_id})
            loser_bets = sum([line[0] for line in last_info
                              if str(line[1]) != winner_id])
            all_bets = sum([line[0] for line in last_info])
            prize = last_bet / (
                    last_bet + all_bets - loser_bets) * loser_bets + last_bet
        ...
        query = 'UPDATE bets SET payout_flag = FALSE ' \
    ...
    msg += text(bold('Your current balance is: '), user_coins, ' coins.',
                sep='')
