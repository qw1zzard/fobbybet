...
async def Check(user_id):
    ...
    next_info = await database.fetch_all(query=query, values={'next_event': next_event, 'user_id': user_id})
    ...
    last_info = await database.fetch_all(query=query, values={'last_event': last_event, 'user_id': user_id})
    ...
    if last_info:
        ...
        if last_driver_id == winner_id and payout_flag == 'True':
            ...
            last_info = await database.fetch_all(query=query, values={'last_event': last_event, 'user_id': user_id})
            loser_bets = sum([line[0] for line in last_info if str(line[1]) != winner_id])
