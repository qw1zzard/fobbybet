...
from config import bot, dp, database, WEBHOOK_URL, WEBHOOK_PATH, WEBAPP_HOST, WEBAPP_PORT
...
async def process_start_command(message: types.Message):
    """
        Handle /start command. Display a welcome message.
    """

    await database.execute('INSERT INTO users (user_id, coins) VALUES (:user_id, 1000.0) ON CONFLICT (user_id) DO NOTHING;',
            values={'user_id': message.from_user.id})

    msg = text('Hi! I\'m Fobby, ', italic('Formula One Betting Bot'), ', yeah.', '\n', 'Use /help to get a list of commands. ',
            'If you are just a beginner, then /check: you already have a thousand coins, spend them wisely!', sep='')
...
async def process_event_command(message: types.Message):
    ...
    msg = text(response['date'], ' - ', response['raceName'], ' at ', [circuit_name], '(', circuit_link, ')', sep='')
    await message.answer(msg, parse_mode=ParseMode.MARKDOWN)
...
async def process_drivers_command(message: types.Message):
    ...
    for i in response:
        msg += text(' ', i['permanentNumber'], '-', i['givenName'], i['familyName'], '\n', sep=' ')

    await message.answer(msg, parse_mode=ParseMode.MARKDOWN)



async def bet(user_id, text):
    """
        Make a bet and enter it into the database.
    """
    ...
    if 0 < bet_value <= user_coins:
        ...
        await database.execute('INSERT INTO bets (user_id, event_date, driver_id, bet_value) '
                'VALUES (:user_id, :event_date, :driver_id, :bet_value);',
                values={'user_id': user_id, 'event_date': next_event, 'driver_id': driver_id, 'bet_value': bet_value})

        await database.execute('UPDATE users SET coins = (coins - :bet_value) WHERE user_id = :user_id;',
...
async def process_bet_command(message: types.Message):
    """
        Handle /check command. Check the bet format and reference to function above.
    """

    if await bet(message.from_user.id, message.text):
        await message.answer('Sports bet accepted! You can check with the command /check.')
    else:
        await message.answer('Please, use the format «/bet number size, ' +
                'where number - driver\'s number (from table /drivers), '
                'and size - positive real size of the bet.')
...
async def process_news_command(message: types.Message):
    ...
    msg = text('It\'s just a beta version of my little personal project.',
            'Feel free to report any bug or suggest an idea: @qw1zzard.', sep=' ')
    await message.answer(msg, parse_mode=ParseMode.MARKDOWN)
...
async def process_help_command(message: types.Message):
    ...
    msg = text(bold('I answer these commands:'),
            ...
            '/help - list of commands.. that you already see', sep='\n')
    await message.answer(msg, parse_mode=ParseMode.MARKDOWN)
