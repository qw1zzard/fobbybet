...
async def process_start_command(message: types.Message):
    ...
    msg = text('Hi! I\'m Fobby, ', italic('Formula One Betting Bot'),
               ...
               'have a thousand coins, spend them wisely!', sep='')
...
async def process_event_command(message: types.Message):
    ...
    msg = text(response['date'], ' - ', response['raceName'], ' at ',
               [circuit_name], '(', circuit_link, ')', sep='')
    await message.answer(msg, parse_mode=ParseMode.MARKDOWN)
...
async def process_drivers_command(message: types.Message):
    ...
    for i in response:
        msg += text(' ', i['permanentNumber'], '-', i['givenName'],
                    i['familyName'], '\n', sep=' ')
...
async def process_news_command(message: types.Message):
    ...
    msg = text('I recently refactored the code to meet PEP 8 standards ',
